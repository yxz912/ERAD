...
texture_list = ['carpet', 'zipper', 'leather', 'tile', 'wood','grid',
                ...
                'breakfast_box','juice_bottle','pushpins','screw_bag','splicing_connectors','zqg']
...
class MVTecTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.image_transform(image)
        augmented_image = self.image_transform(augmented_image)
...
class VisATestDataset(Dataset):
    ...
    def __init__(self, data_path,classname,img_size):
        self.root_dir = os.path.join(data_path,'Data/Images')
...
class VisATrainDataset(Dataset):
    def __init__(self, data_path,classname,img_size,args):
        self.classname=classname
        self.root_dir = os.path.join(data_path,'Data','Images/Normal/')
        self.resize_shape = [img_size[0], img_size[1]]
        self.anomaly_source_path = args["anomaly_source_path"]
        self.anomaly_source_paths = sorted(glob.glob(self.anomaly_source_path + "/images/*/*.jpg"))
    ...
    def get_foreground(self,image_path):
        foreground_path = image_path.replace('Images', 'DISthresh')
        foreground_path = foreground_path.replace('Normal', 'good')
        return foreground_path 
        ...
        if no_anomaly > 0.5:
            ...
            if self.classname in texture_list:
                ...
                if texture_or_patch > 0.5:  
                    aug = self.randAugmenter()
                    asimg = cv2.imread(anomaly_source_path)
                    anomaly_source_img = cv2.cvtColor(asimg,cv2.COLOR_BGR2RGB)
                    anomaly_source_img = cv2.resize(anomaly_source_img, dsize=(
                        self.resize_shape[1], self.resize_shape[0]))
                    anomaly_img_augmented = aug(image=anomaly_source_img)
                    img_object_thr = anomaly_img_augmented.astype(
                    ...
                    # cv2.imshow("asimg",img_object_thr)
                    # cv2.imshow("obj",object_perlin)
            ...
            # cv2.imshow("io",image *(1 - object_perlin))
            # cv2.imshow(" bi ",(1 - beta) *img_object_thr)
            # cv2.imshow(" bio ", beta * image * (object_perlin))
            # cv2.waitKey(0)
    ...
    def __getitem__(self, idx):
        while(1):
            idx = torch.randint(0, len(self.image_paths), (1,)).item()
            image_path = self.root_dir + self.image_paths[idx]
            image = cv2.cvtColor(cv2.imread(image_path),cv2.COLOR_BGR2RGB)
            image = cv2.resize(image, dsize=(self.resize_shape[1], self.resize_shape[0]))
            cv2_image=image
...
class DAGMTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': image, 'has_anomaly': has_anomaly,
                  'mask': mask, 'idx': idx, 'type': img_path[len(self.root_dir):-8],
...
class DAGMTrainDataset(Dataset):
    ...
    def perlin_synthetic(self, image, thresh, anomaly_source_path, cv2_image, thresh_path):
        ...
        if no_anomaly > 0.5:
            image = image.astype(np.float32)
            return image, np.zeros((self.resize_shape[0], self.resize_shape[1], 1), dtype=np.float32), np.array([0.0],
                                                                                                                dtype=np.float32)
...
class MVTec_locoTestDataset(Dataset):
    def __init__(self, data_path, classname, img_size):
        self.root_dir = os.path.join(data_path, 'test')
        self.images = sorted(glob.glob(self.root_dir + "/*/*.png"))
        self.resize_shape = [img_size[0], img_size[1]]
...
class MVTec_locoTrainDataset(Dataset):
    def __init__(self, data_path, classname, img_size, args):
