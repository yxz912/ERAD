...
import warnings
from wavelet import DWT, IWT,DWT_2D
...
def train(training_dataset_loader, testing_dataset_loader, args, data_len,sub_class,class_type,device ):
    ...
    for epoch in tqdm_epoch:
        ...
        for i, sample in enumerate(tbar):
            ...
            # pred_mask = seg_model(aug_image/(pred_x0+10e-8))
        ...
        if (epoch+1) % 10==0 and epoch > 0:
            ...
            if (class_weight*temp_image_auroc + seg_weight*temp_pixel_auroc >= class_weight*best_image_auroc + seg_weight*best_pixel_auroc):
...
def save(unet_model,seg_model, args,final,epoch,sub_class):
    if args["fk"]:
        if final == 'last':
            torch.save(unet_model.state_dict(),f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params_unet-{final}.pt')
            torch.save(seg_model.state_dict(),
                       f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params_seg-{final}.pt')
        else:
            torch.save(unet_model.state_dict(),
                       f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params_unet-{final}.pt')
            torch.save(seg_model.state_dict(),
                       f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}/params_seg-{final}.pt')
    else:
        if final=='last':
...
def main():
    ...
    for sub_class in current_classes:    
        ...
        if sub_class in visa_classes:
            ...
            training_dataset = MVTec_locoTrainDataset(
                subclass_path,sub_class,img_size=args["img_size"],args=args
                )
        ...
        training_dataset_loader = DataLoader(training_dataset, batch_size=args['Batch_Size'],shuffle=True,num_workers=16,pin_memory=True,drop_last=True)
        test_loader = DataLoader(testing_dataset, batch_size=1,shuffle=False, num_workers=16)
