...
from sklearn.metrics import roc_curve, roc_auc_score, confusion_matrix, ConfusionMatrixDisplay,auc,average_precision_score
import time
import numpy as np
...
def image_transform(image):
    ...
    return np.clip(reverse_transforms(image)* 255, 0, 255).astype(np.uint8)
...
def testing(testing_dataset_loader, args,unet_model,seg_model,data_len,sub_class,class_type,checkpoint_type,device):
    ...
    for i, sample in enumerate(tbar):
        ...
        torch.cuda.reset_peak_memory_stats()  # 重置显存计数
        ...
        # 记录峰值显存
        peak_memory = torch.cuda.max_memory_allocated() / 1024 ** 2  # 转为 MB
        peak_memories.append(peak_memory)
        #print(f"Inference time: {inference_time:.4f} s, Peak Memory: {peak_memory:.2f} MB")
    ...
    print("Average Peak Memory (MB):", avg_peak_memory)

    # 计算 ROC 曲线的假阳性率、真正率和阈值
    ...
    # 计算 Youden’s J Statistic，找到最佳阈值
    youden_index = tpr - fpr
    best_threshold_index = np.argmax(youden_index)
    best_threshold = thresholds[best_threshold_index]

    # 计算混淆矩阵
    predictions = (total_image_pred >= best_threshold).astype(int)  # 使用最佳阈值将预测得分转为分类标签
    cm = confusion_matrix(total_image_gt, predictions)
    cm_display = ConfusionMatrixDisplay(confusion_matrix=cm)

    # 绘制 ROC 曲线
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, color='b', label=f'ROC curve (AUROC = {auroc_image}%)')
    plt.plot([0, 1], [0, 1], color='gray', linestyle='--')  # 对角线（随机猜测）
    ...
    # 输出最佳阈值和 AUROC
    print(f"AUROC: {auroc_image}%, Best Threshold: {best_threshold}")
...
def main():
    ...
    MVTec_loco_class = ['breakfast_box', 'juice_bottle', 'pushpins', 'screw_bag', 'splicing_connectors']
    mydata_class = ['zqg']
    
    current_classes=mydata_class
    ...
    for sub_class in current_classes:
        ...
        # num_params1 = sum(p.numel() for p in unet_model.parameters())
        # print(f"unet模型参数量: {num_params1}")
        # num_params2 = sum(p.numel() for p in seg_model.parameters())
        # print(f"seg模型参数量: {num_params2}")
        ...
        if sub_class in visa_classes:
            ...
            class_type='VisA'
        elif sub_class in mydata_class:
            subclass_path = os.path.join(args["mydata_root_path"],sub_class)
        ...
        elif sub_class in MVTec_loco_class:
            subclass_path = os.path.join(args["MVTec_loco_root_path"],sub_class)
            testing_dataset = MVTec_locoTestDataset(
                subclass_path, sub_class, img_size=args["img_size"],
            )
            class_type = 'MVTec_loco'
        
                
        data_len = len(testing_dataset) 
        test_loader = DataLoader(testing_dataset, batch_size=1,shuffle=False, num_workers=8)
