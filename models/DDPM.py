...
class GaussianDiffusionModel:
    ...
    def calc_loss(self, model, x_0, t,args):
        if args["dwt"] ==1:
            LL, LH, HL, HH = dwt(x_0)
            x_0 = torch.cat([LL, LH, HL, HH], dim=1)
            x_0 = x_0 / 2.0
    ...
    def adaptive_thresholding(self,tensor):
        # Convert PyTorch tensor to NumPy array for processing
        tensor_np = tensor.cpu().detach().numpy()

        # Initialize an empty array to store the thresholded result
        thresholded_tensor_np = np.zeros_like(tensor_np)

        # Apply adaptive thresholding to each slice along the first dimension
        for i in range(tensor_np.shape[0]):
            slice_ = tensor_np[i]
            threshold = threshold_otsu(slice_)
            thresholded_slice = (slice_ > threshold).astype(np.float32)
            thresholded_tensor_np[i] = thresholded_slice

        # Convert the thresholded result back to a PyTorch tensor
        thresholded_tensor = torch.from_numpy(thresholded_tensor_np)

        return thresholded_tensor

    def fusion(self,pred_x_0_noisier,x_0,args):
        if args["dwt"] == 1:
            LL, LH, HL, HH = dwt(x_0)
            x_0 = torch.cat([LL, LH, HL, HH], dim=1)
            x_0 = x_0 / 2.0
        eimg = torch.abs(pred_x_0_noisier-x_0)
        eimg01 = self.adaptive_thresholding(eimg).cuda()

        y = eimg01*pred_x_0_noisier + (1-eimg01)*x_0
    ...
    def norm_guided_one_step_denoising(self, model, x_0, anomaly_label,args):
        ...
        normal_loss, x_normal_t, estimate_noise_normal = self.calc_loss(model, x_0, normal_t,args)
        noisier_loss, x_noiser_t, estimate_noise_noisier = self.calc_loss(model, x_0, noisier_t,args)
        
        pred_x_0_noisier = self.predict_x_0_from_eps(x_noiser_t, noisier_t, estimate_noise_noisier).clamp(-1, 1)

        ####IF
        if args["IF"]:
            pred_x_0_noisier = self.fusion(pred_x_0_noisier,x_0,args)
        ...
        if args["dwt"]==1:
            pred_x_0_norm_guided = pred_x_0_norm_guided * 2.0
            pred_x_0_norm_guided = iwt(pred_x_0_norm_guided[:, 0:3, :, :], pred_x_0_norm_guided[:, 3:6, :, :], pred_x_0_norm_guided[:, 6:9, :, :],pred_x_0_norm_guided[:, 9:12, :, :])
            x_normal_t = x_normal_t * 2.0
            x_normal_t = iwt(x_normal_t[:, 0:3, :, :], x_normal_t[:, 3:6, :, :], x_normal_t[:, 6:9, :, :], x_normal_t[:, 9:12, :, :])
            x_noiser_t = x_noiser_t * 2.0
            x_noiser_t = iwt(x_noiser_t[:, 0:3, :, :], x_noiser_t[:, 3:6, :, :], x_noiser_t[:, 6:9, :, :], x_noiser_t[:, 9:12, :, :])
    ...
    def norm_guided_one_step_denoising_eval(self, model, x_0, normal_t,noisier_t,args):
        with torch.no_grad():
            normal_loss, x_normal_t, estimate_noise_normal = self.calc_loss(model, x_0, normal_t,args)
        with torch.no_grad():
            noisier_loss, x_noisier_t, estimate_noise_noisier = self.calc_loss(model, x_0, noisier_t,args)
        ...
        estimate_noise_hat = estimate_noise_normal - extract(self.sqrt_one_minus_alphas_cumprod, normal_t, x_normal_t.shape, x_normal_t.device) * args["condition_w"] * (pred_x_t_noisier-x_normal_t)
        pred_x_0_norm_guided = self.predict_x_0_from_eps(x_normal_t, normal_t, estimate_noise_hat).clamp(-1, 1)
        ...
        if args["dwt"]==1:
            ...
            x_normal_t = x_normal_t * 2.0
            x_normal_t = iwt(x_normal_t[:, 0:3, :, :], x_normal_t[:, 3:6, :, :], x_normal_t[:, 6:9, :, :], x_normal_t[:, 9:12, :, :])
            pred_x_0_normal = pred_x_0_normal * 2.0
            pred_x_0_normal = iwt(pred_x_0_normal[:, 0:3, :, :], pred_x_0_normal[:, 3:6, :, :], pred_x_0_normal[:, 6:9, :, :], pred_x_0_normal[:, 9:12, :, :])
            pred_x_0_noisier = pred_x_0_noisier * 2.0
            pred_x_0_noisier = iwt(pred_x_0_noisier[:, 0:3, :, :], pred_x_0_noisier[:, 3:6, :, :], pred_x_0_noisier[:, 6:9, :, :],pred_x_0_noisier[:, 9:12, :, :])
            pred_x_t_noisier = pred_x_t_noisier  * 2.0
            pred_x_t_noisier = iwt(pred_x_t_noisier[:, 0:3, :, :], pred_x_t_noisier[:, 3:6, :, :],pred_x_t_noisier[:, 6:9, :, :], pred_x_t_noisier[:, 9:12, :, :])
            x_noisier_t = x_noisier_t * 2.0
            x_noisier_t = iwt(x_noisier_t[:, 0:3, :, :], x_noisier_t[:, 3:6, :, :],x_noisier_t[:, 6:9, :, :], x_noisier_t[:, 9:12, :, :])
    ...
    def noise_t(self, model, x_0, t,args):
        loss, x_t, estimate_noise = self.calc_loss(model, x_0, t,args)
        loss = (loss["loss"] ).mean()
        pred_x_0 = self.predict_x_0_from_eps(x_t, t, estimate_noise).clamp(-1, 1)
